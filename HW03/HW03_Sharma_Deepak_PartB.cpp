/*
* @file     HW03_Sharma_Deepak_PartB.cpp
...
* diving_video_far__board_477E38120CAF.MOV
...
void processVideo(char* videoFileName) {
    ...
    imwrite("./HW03_Sharma_Deepak_PartB_OUTPUT.jpg", output);
...
void processForeground(Mat &fgMaskMOG2, Mat &cleanFgMask){
    ...
    for (int label = 1; label < nLabels; ++label){
        ...
        if (area > MIN_AREA  && area < MAX_AREA && area > maxArea &&\
                !(HWratio > ASPACT_RATIO || WHratio > ASPACT_RATIO)){
            largestLabel = label;
            maxArea = area;}}
    ...
    //Performed closing operation for filling completly the traget connected
    //component
